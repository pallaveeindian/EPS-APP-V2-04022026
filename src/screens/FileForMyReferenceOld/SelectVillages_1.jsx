// .\src\screens\screensProductionApp\SelectVillages.jsx
...
export default function SelectVillage({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    const fetchVillages = async () => {
      setLoading(true);
      try {
        // Support pagination + search query
        const res = await gsApi.getVillagesByPanchayat(adminPanchayatId, page, query);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.listItem}>
            <Text style={styles.listText}>{item.village_name_english}</Text>
            <TouchableOpacity
              style={styles.openButton}
              onPress={() =>
                navigation.navigate('SelectShg', { adminVillageId: item.village_id })
              }
            >
