...
//     // --- RENAME addr TO beneficiaryAddr TO PREVENT SCOPE COLLISIONS ---
...
//     // Use beneficiaryAddr instead of addr
//     const addressText =
//       (beneficiaryAddr?.address_line1 && String(beneficiaryAddr.address_line1).trim()) ||
//       (beneficiaryAddr?.address_line2 && String(beneficiaryAddr.address_line2).trim()) ||
//       "";

//     const age = computeAgeFromDob(beneficiary.dob);

//     let district_id = beneficiaryAddr?.district_id ?? beneficiaryAddr?.districtId ?? null;
//     let block_id = beneficiaryAddr?.block_id ?? beneficiaryAddr?.blockId ?? null;
//     let panchayat_id = beneficiaryAddr?.panchayat_id ?? beneficiaryAddr?.panchayatId ?? null;
//     let village_id = beneficiaryAddr?.village_id ?? beneficiaryAddr?.villageId ?? null;
...
//     // 3. Fallback logic remains the same but uses updated variable names
//     if (
//       (!district_id || !block_id || !panchayat_id || !village_id || !lokos_shg) &&
//       tempShg
//     ) {
//       const loc = extractLocationFromShg(tempShg);
//       if (loc) {
...
//     setSubmitting(true);
//     try {
//       // 1) Ensure recorded beneficiary exists
//       let recordedBenefId =
//         recordedBenef?.id || existingEnterprise?.recorded_benef_id || null;
//       if (!recordedBenefId) {
//         try {
//           recordedBenefId = await ensureRecordedBeneficiary();
//         } catch (e) {
...
//       // --- ADD THIS BLOCK TO FIX THE 'addr' ERROR ---
//       const localAddr = Array.isArray(beneficiary?.member_addresses) &&
//         beneficiary.member_addresses.length > 0
//         ? beneficiary.member_addresses[0] : null;

//       const localPhone = Array.isArray(beneficiary?.member_phones) &&
//         beneficiary.member_phones.length > 0
//         ? beneficiary.member_phones[0] : null;
//       // ----------------------------------------------

//       // 2) Sanitize sales data (existing logic...)
//       const sanitizedProducts = safeArray(existingForm?.products).map(
//         (prod) => {
//           const monthly = parseFloat(prod.avg_monthly_sales || 0);
//           return {
//             ...prod,
...
//       // 3) Build payload - UPDATED TO USE localAddr and localPhone
//       const finalPayload = {
//         beneficiary: {
//           lokos_member_code: memberCode,
//           applicant_name: beneficiary?.member_name || "",
//           age: computeAgeFromDob(beneficiary?.dob),
//           gender: beneficiary?.gender || "",
...
//           education: beneficiary?.education || "",
//           // FIX: Changed beneaddr to localAddr
//           address: localAddr?.address_line1 || beneficiary?.address || "",
//           // FIX: Changed addr to localAddr
//           district_id: localAddr?.district_id || localAddr?.districtId || null,
//           block_id: localAddr?.block_id || localAddr?.blockId || null,
//           panchayat_id: localAddr?.panchayat_id || localAddr?.panchayatId || null,
//           village_id: localAddr?.village_id || localAddr?.villageId || null,
//           // FIX: Changed phone to localPhone
//           mobile: localPhone?.phone_no || localPhone?.mobile || beneficiary?.mobile || "",
//           email: beneficiary?.email || "",
//           lokos_shg_code:
//             beneficiary?.lokos_shg_code ||
//             tempShg?.code ||
//             tempShg?.lokos_shg_code ||
//             "",
//           created_by: getCreatedByNumeric(),
//         },
...
//           owner_cadre: Array.isArray(payload.owner_cadre)
//             ? payload.owner_cadre.join(", ")
//             : payload.owner_cadre || "",
//           owner_designation: Array.isArray(payload.owner_designation)
//             ? payload.owner_designation.join(", ")
//             : payload.owner_designation || "",
//           owner_special_category: payload.owner_special_category || "",

//           year_of_establishment: sanitizeNumberField(
//             payload.year_of_establishment,
//           ),
//           total_emp: sanitizeNumberField(payload.total_emp),
//           number_of_shg_emp: sanitizeNumberField(payload.number_of_shg_emp),

//           workplace_type: payload.workplace_type || "",
//           electricity_available: normalizeBoolean(
//             payload.electricity_available,
//           ),
//           water_available: normalizeBoolean(payload.water_available),
//           transportation_availability: normalizeBoolean(
//             payload.transportation_availability,
//           ),
//           can_send_to_bijnor: normalizeBoolean(payload.can_send_to_bijnor),
//           need_transport_help: normalizeBoolean(payload.need_transport_help),
//           have_shop_based_prod: normalizeBoolean(payload.has_shop_product),

//           monthly_income_estimate: sanitizeDecimalField(
//             payload.monthly_income_estimate,
//           ),
//           annual_turnover: sanitizeDecimalField(payload.annual_turnover),
//           gross_profit: sanitizeDecimalField(payload.gross_profit),
//           working_capital_monthly: sanitizeDecimalField(
//             payload.working_capital_monthly,
//           ),
//           initial_investment: sanitizeDecimalField(payload.initial_investment),
//           source_of_investment: payload.source_of_investment || "",

//           has_taken_loan: normalizeBoolean(payload.has_taken_loan),
//           has_receieved_subsidy: normalizeBoolean(
//             payload.has_receieved_subsidy || payload.has_received_subsidy,
//           ),
//           has_shg_receieved_man_fund: normalizeBoolean(payload.has_shg_cif), // Uses has_shg_cif from state

//           is_training_received: normalizeBoolean(payload.is_training_received),
//           is_training_required: normalizeBoolean(payload.is_training_required),
//           expansion_plan: payload.expansion_plan || "",
//           info_abt_gov_scheme: normalizeBoolean(payload.info_abt_gov_scheme),
//           nearest_skill_centre: payload.nearest_skill_centre || "",
//           skill_centre_loc: payload.skill_centre_loc || "",
//           nearest_industry: payload.nearest_industry || "",
//           industry_loc: payload.industry_loc || "",
//           is_support_required: normalizeBoolean(payload.is_support_required),
...
//         licenses: safeArray(safeLicenses).map((lic, i) => ({
//           license_category: lic.license_category || "",
...
//         loans: safeArray(loans).map((loan) => ({
//           form_type: "exep",
//           department: loan.department || "",
//           institution_name: loan.institution_name || "",
//           bank_name:
...
//         subsidies: safeArray(subsidies).map((sub) => {
//           let subsidyTypeText = "";
//           let subsidyNameText = "";
//           if (
//             Array.isArray(sub.subsidy_name_tree) &&
...
//         // Only populate shop if user selected YES
//         shops:
//           payload.has_shop_product === "Yes"
//             ? [
//               {
//                 shop_category: payload.shop_sub_category || "",
//                 shop_type: payload.shop_type || "",
//                 source_of_inventory: payload.inventory_source || "",
//                 target_customers: Array.isArray(payload.target_customers)
//                   ? payload.target_customers.join(", ")
//                   : payload.target_customers || "",
//                 sales_area: Array.isArray(payload.sales_area)
//                   ? payload.sales_area.join(", ")
//                   : payload.sales_area || "",
//                 marketing_strategy: Array.isArray(payload.marketing_strategy)
//                   ? payload.marketing_strategy.join(", ")
//                   : payload.marketing_strategy || "",
//                 marketing_channels: Array.isArray(payload.marketing_channels)
//                   ? payload.marketing_channels.join(", ")
//                   : payload.marketing_channels || "",
//                 marketing_challenges: Array.isArray(
//                   payload.marketing_challenges,
//                 )
//                   ? payload.marketing_challenges.join(", ")
//                   : payload.marketing_challenges || "",
//                 market_linkage: normalizeBoolean(payload.market_linkage),
//                 accept_digital_payment: normalizeBoolean(
//                   payload.accept_digital_payment,
//                 ),
//                 avg_monthly_sales: payload.avg_monthly_sales || "0.00",
//                 avg_annual_sales: payload.annual_sale || "0.00", // Note: your state uses annual_sale for shops
//                 media: {
//                   front_key: "shop_front_0",
//                   inside_key: "shop_inside_0",
//                   others_key: "shop_others_0",
//                 },
//                 created_by: createdBy,
//               },
//             ]
...
//           packaging_branding_status: normalizeBoolean(
//             prod.packaging_branding_status,
//           ),
...
//           market_linkage: normalizeBoolean(prod.market_linkage),
...
//           media: {
//             open_box_key: `prod_${i}_open`,
//             close_box_key: `prod_${i}_close`,
//             others_key: `prod_${i}_others`,
//           },
//           created_by: createdBy,
//         })),

//         enterprise_media: {
//           entrepreneur_key: "photo_entrepreneur",
//           enterprise_key: "photo_enterprise",
//           others_key: "media_others",
//           created_by: createdBy,
//         },

//         categories: payload.enterprise_types_tree
//           ? payload.enterprise_types_tree.map((cat) => ({
//             parent_category: cat.parent?.en || "",
//             sub_category: cat.children
//               ? cat.children.map((c) => c.en).join(", ")
//               : "",
//             created_by: createdBy,
//           }))
//           : [],
...
//         supports: (() => {
//           const sData = payload.support_required || {};
//           const mapped = [];
//           if (sData.financial)
//             mapped.push({
//               category: "Financial",
...
//         mandatory_funds: safeArray(payload.fund_cards).map((fund) => {
...
//         training_requests: [
//           ...safeArray(trainingReceivedRows).map((row, i) => ({
//             form_type: "rec",
...
//             created_by: createdBy,
//           })),
//           ...safeArray(trainingRequiredRows).map((row) => ({
//             form_type: "req",
//             sector_type: row.sector_tree?.[0]?.parent || "",
//             sector: row.sector_tree?.[0]?.children?.join(", ") || "",
//             department: row.department || "",
...
//       // 6) Append ALL files safely mapping to the keys established above
//       safeArray(safeLicenses).forEach((lic, i) => {
//         if (lic.file) {
//           formData.append(`license_${i}`, {
//             uri: lic.file.uri,
//             type: lic.file.type || "image/jpeg",
//             name: lic.file.fileName || `license_${i}.jpg`,
//           });
//         }
//       });

//       // Shop Media (Only if shop product is yes)
//       if (payload.has_shop_product === "Yes" && payload.media) {
//         if (payload.media.shop_front?.[0])
//           formData.append("shop_front_0", {
//             uri: payload.media.shop_front[0].uri,
//             type: payload.media.shop_front[0].type || "image/jpeg",
//             name: "shop_front.jpg",
//           });
//         if (payload.media.shop_inside?.[0])
//           formData.append("shop_inside_0", {
//             uri: payload.media.shop_inside[0].uri,
//             type: payload.media.shop_inside[0].type || "image/jpeg",
//             name: "shop_inside.jpg",
//           });
//       }
...
//         if (media.open_box?.[0])
//           formData.append(`prod_${i}_open`, {
//             uri: media.open_box[0].uri,
//             type: media.open_box[0].type || "image/jpeg",
//             name: `prod_${i}_open.jpg`,
//           });
//         if (media.close_box?.[0])
//           formData.append(`prod_${i}_close`, {
//             uri: media.close_box[0].uri,
//             type: media.close_box[0].type || "image/jpeg",
//             name: `prod_${i}_close.jpg`,
//           });
//         if (media.others?.[0])
//           formData.append(`prod_${i}_others`, {
//             uri: media.others[0].uri,
//             type: media.others[0].type || "image/jpeg",
//             name: `prod_${i}_others.jpg`,
//           });
//       });

//       // Enterprise Media
//       const epMedia = payload.media || {};
//       if (epMedia.photo_entrepreneur?.[0])
//         formData.append("photo_entrepreneur", {
//           uri: epMedia.photo_entrepreneur[0].uri,
//           type: "image/jpeg",
//           name: "entrepreneur.jpg",
//         });
//       if (epMedia.photo_enterprise?.[0])
//         formData.append("photo_enterprise", {
//           uri: epMedia.photo_enterprise[0].uri,
//           type: "image/jpeg",
//           name: "enterprise.jpg",
//         });
//       if (epMedia.others?.[0])
//         formData.append("media_others", {
//           uri: epMedia.others[0].uri,
//           type: "image/jpeg",
//           name: "others.jpg",
//         });

//       // Training certs
//       safeArray(trainingReceivedRows).forEach((tr, i) => {
//         const files = tr.files || tr.certificates_files || [];
//         files.forEach((file, idx) => {
//           formData.append(`cert_${i}_${idx}`, {
//             uri: file.uri,
//             type: file.type || "image/jpeg",
//             name: file.fileName || `cert_${i}_${idx}.jpg`,
//           });
//         });
//       });
...
// // 🛠️ FIX: Added missing normalizeBoolean function that was causing crashes
// const normalizeBoolean = (val) => {
//   if (val === "Yes" || val === true || val === "true" || val === 1) return true;
//   if (val === "No" || val === false || val === "false" || val === 0) return false;
//   return false;
// };
...
//       water_available: existingEnterprise.water_available || "",
//       licenses: existingEnterprise.licenses || [],
//       transportation_availability: existingEnterprise.transportation_availability || "",
//       can_send_to_bijnor: existingEnterprise.can_send_to_bijnor || "",
//       monthly_income_estimate: existingEnterprise.monthly_income_estimate || "",
//       annual_turnover: existingEnterprise.annual_turnover || "",
//       gross_profit: existingEnterprise.gross_profit || "",
...
//       // 🛠️ FIX: source_of_investment was omitted previously. We stringify it from the tree here.
//       const sourceOfInvestmentString = safeArray(existingForm.source_of_investment_tree)
//         .filter((row) => row.parent && Array.isArray(row.children) && row.children.length > 0)
//         .map((row) => `${row.parent}: ${row.children.join(", ")}`)
//         .join(" | ");

//       // 🛠️ FIX: Replaced ALL `payload.` references with `existingForm.` since `payload` was undefined here.
//       const finalPayload = {
//         beneficiary: {
//           lokos_member_code: memberCode,
//           applicant_name: beneficiary?.member_name || "",
...
//         enterprise: {
//           recorded_benef_id: recordedBenefId && !isNaN(recordedBenefId) ? parseInt(recordedBenefId, 10) : null,
...
//           number_of_shg_emp: sanitizeNumberField(existingForm.number_of_shg_emp),

//           workplace_type: existingForm.workplace_type || "",
//           electricity_available: normalizeBoolean(existingForm.electricity_available),
//           water_available: normalizeBoolean(existingForm.water_available),
//           transportation_availability: normalizeBoolean(
//             existingForm.transportation_availability
//           ),
//           can_send_to_bijnor: normalizeBoolean(existingForm.can_send_to_bijnor),
//           need_transport_help: normalizeBoolean(existingForm.need_transport_help),
...
//           ),
//           initial_investment: sanitizeDecimalField(existingForm.initial_investment),
//           source_of_investment: sourceOfInvestmentString || existingForm.source_of_investment || "",

//           has_taken_loan: normalizeBoolean(existingForm.has_taken_loan),
//           has_receieved_subsidy: normalizeBoolean(
//             existingForm.has_receieved_subsidy || existingForm.has_received_subsidy
//           ),
//           has_shg_receieved_man_fund: normalizeBoolean(existingForm.has_shg_cif),

//           is_training_received: normalizeBoolean(existingForm.is_training_received),
//           is_training_required: normalizeBoolean(existingForm.is_training_required),
//           expansion_plan: existingForm.expansion_plan || "",
//           info_abt_gov_scheme: normalizeBoolean(existingForm.info_abt_gov_scheme),
...
//           is_support_required: normalizeBoolean(existingForm.is_support_required),
...
//         // 🛠️ FIX: Replaced `safeLicenses` (which was undefined) with `existingForm.licenses`
//         licenses: safeArray(existingForm.licenses).map((lic, i) => ({
//           license_category: lic.license_category || "",
//           license_name: lic.license_name || "",
//           license_no: lic.license_no || "",
//           file_key: `license_${i}`,
...
//                 shop_type: existingForm.shop_type || "",
//                 source_of_inventory: existingForm.inventory_source || "",
//                 target_customers: Array.isArray(existingForm.target_customers)
//                   ? existingForm.target_customers.join(", ")
...
//                 marketing_strategy: Array.isArray(existingForm.marketing_strategy)
//                   ? existingForm.marketing_strategy.join(", ")
...
//                   : existingForm.marketing_challenges || "",
//                 market_linkage: normalizeBoolean(existingForm.market_linkage),
//                 accept_digital_payment: normalizeBoolean(
...
//       // 🛠️ FIX: Replaced `safeLicenses` with `existingForm.licenses`
//       safeArray(existingForm.licenses).forEach((lic, i) => {
//         if (lic.file) {
//           formData.append(`license_${i}`, {
//             uri: lic.file.uri,
//             type: lic.file.type || "image/jpeg",
...
//         if (existingForm.media.shop_front?.[0])
//           formData.append("shop_front_0", {
//             uri: existingForm.media.shop_front[0].uri,
//             type: existingForm.media.shop_front[0].type || "image/jpeg",
//             name: "shop_front.jpg",
//           });
//         if (existingForm.media.shop_inside?.[0])
//           formData.append("shop_inside_0", {
//             uri: existingForm.media.shop_inside[0].uri,
//             type: existingForm.media.shop_inside[0].type || "image/jpeg",
//             name: "shop_inside.jpg",
//           });
//       }
...
//       // Enterprise Media
//       // 🛠️ FIX: Referencing `existingForm.media` correctly.
...
//       // 🛠️ FIX: Added declaration of `signature` which was completely missing before.
//       const signature = existingForm?.media?.declaration_signature || [];
//       if (signature?.[0]) {
//         formData.append("signature", {
//           uri: signature[0].uri,
...
export default function ExistingEnterpriseForm({ route, navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 🛠️ FIX: Added fallback to pull existing licenses if LicenseSelector returned empty
      const activeLicenses = existingForm.licenses?.length > 0
      ...
      const finalPayload = {
        beneficiary: {
          ...
          mobile: localPhone?.phone_no || localPhone?.mobile || beneficiary?.mobile || "",
          email: beneficiary?.email || "",
          // 🛠️ FIX: `lokos_shg_code` was coming empty. Added comprehensive fallbacks.
          lokos_shg_code:
            existingEnterprise?.lokos_shg_code ||
            recordedBenef?.lokos_shg_code ||
            beneficiary?.lokos_shg_code ||
        ...
        enterprise: {
          ...
          transportation_availability: existingForm.transportation_availability || "",
          ...
          has_received_subsidy: normalizeBoolean(
            existingForm.has_received_subsidy || existingForm.has_received_subsidy
          ),
            ...
            ? [
              {
                ...
                market_linkage: existingForm.market_linkage || "",
                accept_digital_payment: normalizeBoolean(
                  existingForm.accept_digital_payment
                ),
                avg_monthly_sales: existingForm.avg_monthly_sales || "0.00",
