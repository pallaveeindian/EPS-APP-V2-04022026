...
const toggleInCommaString = (current, option) => {
  ...
  if (arr.includes(option)) return arr.filter(v => v !== option).join(',');
  return [...arr, option].join(',');
};
...
export default function ShopBasedProductSection({ row, index, updateRow }) {
  ...
  const renderMultiCheckbox = (field, optionObj) => {
    ...
    const label = optionObj.en.includes('Others')
      ? language === 'hi'
        ? 'अन्य'
        : 'Others'
      : language === 'hi'
            ...
            <View key={mainChan.en} style={styles.nestedSection}>
              ...
              {isMainSelected && subOptions.length > 0 && (
                <View style={styles.subOptionsContainer}>
                  {subOptions.map(subChan => {
                    // Create unique value: e.g., "Retail_Others"
                    const uniqueSubValue =
                      subChan.en === 'Others'
                        ? `${mainChan.en}_Others`
                        : subChan.en;
                    const isSubSelected = splitMulti(
                      row.marketing_channels,
                    ).includes(uniqueSubValue);
                    ...
                    return (
                      <View key={subChan.en}>
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            updateRow(index, {
                              marketing_channels: toggleInCommaString(
                                row.marketing_channels,
                                uniqueSubValue,
                              ),
                            });
                        ...
                        >
                          ...
                          >
                            {language === 'hi' ? subChan.hi : subChan.en}
                          </Text>
                        </TouchableOpacity>

                        {/* INPUT FOR THIS SPECIFIC 'OTHERS' */}

                        {isSubSelected &&
                          (subChan.en.includes('_Others') ||
                            subChan.en === 'Others') && (
                            <TextInput
                              style={[
                                styles.input,
      ...
      {row.has_shop_product === 'Yes' && (
        <View>
          ...
          <View style={styles.fieldBlock}>
            ...
            {/* IF SHOP TYPE IS OTHERS - SHOW TEXT INPUT DIRECTLY */}
            {row.shop_type === 'Others' && (
              <TextInput
                style={[styles.input, { marginTop: 10 }]}
                placeholder={
                  language === 'hi'
                    ? 'अन्य दुकान का प्रकार बताएं'
                    : 'Specify other shop type'
                }
                value={row.shop_type_other || ''}
                onChangeText={v => updateRow(index, { shop_type_other: v })}
              />
            )}

            {/* SHOP SUBTYPE (Only show if a valid category with children is selected) */}
            {row.shop_type && row.shop_type !== 'Others' && (
          ...
          <View style={styles.fieldBlock}>
            ...
            {/* Use a dedicated state key for text input to prevent overwriting checkbox data */}
