...
const ChipRow = ({ value, options, onChange }) => (
  <View style={styles.chipRow}>
    {options.map(opt => {
      ...
      const optionLabel = typeof opt === 'string' ? opt : opt.label || opt.en;
...
const computeTitleFromTree = (tree, fallback, language) => {
  ...
  const matched = TRAINING_SECTOR_TREE.find(g => g.parent.en === parentEn);

  if (!matched) return fallback;

  return language === 'hi' ? matched.parent.hi : matched.parent.en;
};
...
export default function ExistingEnterpriseTrainingSkillsSection({
  ...
  const updateTrainingRequiredRow = (index, patch) => {
    const next = trainingRequired.map((row, i) =>
      i === index ? { ...row, ...patch } : row,
    );
    updateTrainingRequired(next);
  };
    ...
    <View style={styles.sectionContainer}>
                ...
                <View style={styles.cardBody}>
                  ...
                  <View style={styles.fieldBlock}>
                    ...
                    {TRAINING_TYPE_OPTIONS.map(opt => {
                      ...
                      const selected = (row.training_type || '')
                        .split(',')
                        .includes(value);
                  ...
                  <View style={styles.fieldBlock}>
                    ...
                    <Text style={styles.label}>
                      {language === 'hi'
                        ? 'प्रशिक्षण के लिए आपका पसंदीदा स्थान'
                        : 'Preferred training location'}
                    </Text>

                    {/* 1. Pick the Type */}
                    <Text style={styles.smallLabel}>
                      {language === 'hi' ? 'स्थान का प्रकार' : 'Location Type'}
                    </Text>
                    <View style={[styles.input, { marginBottom: 8 }]}>
                      <Picker
                        // This binds directly to the 'location' field
                      ...
                      >
                        ...
                        />
                        <Picker.Item
                          label={language === 'hi' ? 'ग्राम' : 'Village'}
                          value="Village"
                        />
                      </Picker>
                    </View>
            ...
            {existingForm.nearest_skill_centre === 'Yes' && (
              <View style={{ marginTop: 6 }}>
                ...
                <TextInput
                  style={styles.input}
                  placeholder={
                    language === 'hi' ? 'स्थान दर्ज करें' : 'Enter location'
                  }
      ...
      <View style={[styles.fieldBlock, { marginTop: 18 }]}>
        ...
        <TextInput
          ...
          multiline
          placeholder={
            language === 'hi' ? 'अपनी योजना यहाँ लिखें' : 'Write your plan here'
          }
